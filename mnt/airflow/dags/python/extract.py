...
def extract_app():
    ...
    def brands():
        ...
        #Criação de arquivo para utilizar na consulta no site web motors
        brands_webmotors = [brand.split(' (')[0]
                        .replace(" ", "-")
                        .replace("š", "s")
                        .lower() for brand in vehicule_brand]
        ...
        with open('/opt/airflow/dags/python/data/brands_webmotors.json', 'w', encoding='utf-8') as file:
            json.dump(brands_webmotors, file, ensure_ascii=False, indent=4)

        with open('/opt/airflow/dags/python/data/brands_wikipedia.json', 'w', encoding='utf-8') as file:
        ...
        if brand == "Ford":
            ...
            if models:
                vehicule_brand_model = [
                    ...
                    .replace("ú", "u").replace('ò', 'o')
                    .replace(' ', '-')
                    .lower().replace("alfa-romeo-", "").split('-(')[0]  # .lower() pode ser usado se necessário
    ...
    caminho = "/opt/airflow/dags/python/data/brands_wikipedia.json"
    ...
    for name in data:
        try:
            ...
            brands_models.append({"Brand": name, "Model": vehicle_model})
        except Exception as e:
            print(f"An error occurred for brand '{name}': {e}")


    # Escrever a lista em um arquivo JSON
    with open('/opt/airflow/dags/python/data/brands_models.json', 'w', encoding='utf-8') as file:
        json.dump(brands_models, file, ensure_ascii=False, indent=4)

    print("Lista salva como JSON!")
