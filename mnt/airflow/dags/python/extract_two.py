...
def extract_two():
  ...
  def get_model_year(brand, models):
    ...
    def modelo(model):
      """
      Função interna que chama `get_year()` para um modelo e adiciona os dados ao dicionário `brand_data`.

      Parâmetros:
          model (str): Nome do modelo de veículo.
      ...
      if year:
        # Adiciona os dados diretamente nas listas do dicionário
    ...
    if brand_data['Year'] == []:
      pass
        ...
        return df.to_json(caminho_arquivo, orient='index')
